// Portable render + audit for the Precision-Crop-Genomics product page.
// Edit the playwright import path / CHROME for your env.
// Usage: serve index.html via local HTTP (so /favicon.ico resolves), then `node render.mjs`.
// Or: PAGE_URL='file:///path/to/index.html' node render.mjs
...
await page.screenshot({ path: `${OUT_DIR}/screenshots/full-1440.png`, fullPage: true });
...
await page.screenshot({ path: `${OUT_DIR}/screenshots/top-crop.png`, clip: clamp(0, 1200) });

const insightTop = await page.$eval('main .b-cell.b-title.is-centered.cell-3', el => el.getBoundingClientRect().top + window.scrollY);
const compBottom = await page.$eval('main .b-fig.cell-8', el => { const r = el.getBoundingClientRect(); return r.top + window.scrollY + r.height; });
await page.screenshot({ path: `${OUT_DIR}/screenshots/insight-benchmark-crop.png`, clip: clamp(insightTop - 60, Math.round(compBottom - insightTop + 120)) });
...
const unlockBottom = await page.$eval(unlockRow, el => { const r = el.getBoundingClientRect(); return r.top + window.scrollY + r.height; });
await page.screenshot({ path: `${OUT_DIR}/screenshots/possibility-crop.png`, clip: clamp(possTop - 40, Math.round(unlockBottom - possTop + 100)) });
...
const audit = await page.evaluate(() => {
  ...
  return {
    ...
    headline, liveLane, cellIds: all, placeholders, fiveValues,
...
await mob.page.screenshot({ path: `${OUT_DIR}/screenshots/mobile-414.png`, fullPage: true });
await mob.ctx.close();
