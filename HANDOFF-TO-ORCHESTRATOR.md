...
- **`RESISTANCE.md`** — Anti-corruption doctrine. Read first. Augmented per Polymath Diffusion / RCE precedents with `fp-pretty-greenflag` and `fp-less-work-disguised-as-focus`. **The operator's 2026-05-02 "no false flag reward hacking or interim reporting — this is overnight execution" directive reinforces these as binding.** The discipline IS the resistance.
...
- **RESISTANCE.md doctrine.** Every executor reads it before starting.
...
- **Anti-decision pre-registration** (the eighteen anti-decisions enumerated in MODUS-OPERANDI.md; any sub-brief proposing one fails brain-functionality at PRD review).
...
- RESISTANCE.md doctrine carried through, augmented with fp-pretty-greenflag and fp-less-work-disguised-as-focus.
